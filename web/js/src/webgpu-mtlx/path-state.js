  ...
  for(var step=0u;step<4u;step++) {
    ...
    if(mediumDepth>0u && mediumMajorant(p.media[mediumDepth]-1u)>0.0) {
      ...
      if(distance<h.t) {
        ...
        let ctx=ShadingContext(point,-p.direction.xyz,vec3f(1,0,0),vec3f(0,1,0),vec2f(0),0,0,vec2f(0),vec2f(0),vec3f(1,0,0),vec3f(0,1,0),-p.direction.xyz);
    ...
    if(mediumDepth>0u && homogeneous) {
      let mediumContext=ShadingContext(p.origin.xyz,-p.direction.xyz,vec3f(1,0,0),vec3f(0,1,0),vec2f(0),0,0,vec2f(0),vec2f(0),vec3f(1,0,0),vec3f(0,1,0),-p.direction.xyz);
