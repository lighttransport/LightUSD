...
fn validClosure(c:Closure)->bool {
  ...
  for(var i=0u;i<c.count;i++){
    ...
    if(!all(l.base>=vec3f(0))||!all(l.base<=vec3f(3e37))||!all(l.transmissionColor>=vec3f(0))||!all(l.transmissionColor<=vec3f(3e37))||!all(l.transmissionScatter>=vec3f(0))||!all(l.transmissionScatter<=vec3f(3e37))||!all(l.complexIOR>=vec3f(0))||!all(l.complexIOR<=vec3f(3e37))||!all(l.extinction>=vec3f(0))||!all(l.extinction<=vec3f(3e37))||!(l.metal>=0.0&&l.metal<=1.0&&l.transmission>=0.0&&l.transmission<=1.0)){return false;}
