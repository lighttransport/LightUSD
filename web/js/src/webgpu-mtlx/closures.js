...
struct Material { bsdf:Closure, emission:vec3f, opacity:f32, normal:vec3f, emissionDirection:vec3f, emissionInnerCos:f32, emissionOuterCos:f32, emissionCone:u32 }
...
fn surfaceEmission(bsdf:Closure,edf:vec3f,opacity:f32,thinWalled:u32,normal:vec3f,emissionDirection:vec3f,emissionInnerCos:f32,emissionOuterCos:f32,emissionCone:u32)->Material {var c=bsdf;for(var i=0u;i<c.count;i++){c.lobes[i].thinWalled=thinWalled;}return Material(c,edf,opacity,normal,emissionDirection,emissionInnerCos,emissionOuterCos,emissionCone);}
fn materialFromLobe(lobe:Lobe,opacity:f32,normal:vec3f)->Material {return Material(closureLeaf(lobe),lobe.emission*lobe.emissionWeight,opacity,normal,normal,-1.0,-1.0,0u);}
fn materialFromClosure(bsdf:Closure,emission:vec3f,opacity:f32,normal:vec3f)->Material {return Material(bsdf,emission,opacity,normal,normal,-1.0,-1.0,0u);}
fn emissionFactor(m:Material,direction:vec3f)->f32 {if(m.emissionCone==0u){return 1.0;}let c=dot(normalize(m.emissionDirection),normalize(direction));return smoothstep(m.emissionOuterCos,m.emissionInnerCos,c);}
