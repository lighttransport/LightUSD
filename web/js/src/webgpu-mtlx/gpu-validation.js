...
export async function validateValueKernels(device) {
  ...
  const module = device.createShaderModule({ code: `${contextWGSL}\n@group(0) @binding(0) var<storage,read_write> result: array<f32>; @compute @workgroup_size(1) fn main() { let ctx=ShadingContext(vec3f(0),vec3f(0,0,1),vec3f(1,0,0),vec3f(0,1,0),vec2f(0),0,0,vec2f(0),vec2f(0),vec3f(1,0,0),vec3f(0,1,0),vec3f(0,0,1),vec4f(0,0,0,1),vec4f(0),vec4f(0),vec4f(0)); ${bodies.join('\n')} }` });
