    ...
    if (compound) {
      ...
      switch (n.category) {
        ...
        case 'triplanarprojection': {
          ...
          const descriptor=files.map(file=>{const p=ins[file];if(!p?.value||p.nodename||p.nodegraph||p.interfacename)fail('RESOURCE',key,`${file} requires a static resolved filename`);const d=Object.hasOwn(imageDescriptors,p.value)&&imageDescriptors[p.value];if(!d)fail('RESOURCE',key,`missing decoded image ${p.value}`);return {file,p,d};});
          ...
          const weights=`abs(${nrm})/max(1e-6,dot(abs(${nrm}),vec3f(1.0)))`;code=`${samples[0]}*${weights}.x+${samples[1]}*${weights}.y+${samples[2]}*${weights}.z`;break;
        }
        ...
        case 'absval': case 'sign': case 'floor': case 'ceil': case 'round': case 'sqrt': case 'ln': case 'exp': case 'exp2': case 'sin': case 'cos': case 'tan': case 'asin': case 'acos': case 'atan': case 'normalize': code = `${({ absval: 'abs', ln: 'log' })[n.category] || n.category}(${same('in')})`; break;
