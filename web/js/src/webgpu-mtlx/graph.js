    ...
    if (compound) {
      ...
      switch (n.category) {
        ...
        case 'standard_surface': case 'open_pbr_surface': {
          ...
          const supported = new Set(open ? ['base_weight', 'base_color', 'base_diffuse_roughness', 'base_metalness', 'specular_weight', 'specular_color', 'specular_roughness', 'specular_ior', 'specular_roughness_anisotropy', 'transmission_weight', 'transmission_color', 'transmission_depth', 'transmission_scatter', 'subsurface_weight', 'subsurface_color', 'subsurface_radius', 'coat_weight', 'coat_color', 'coat_roughness', 'coat_ior', 'emission_color', 'emission_luminance', 'geometry_opacity', 'geometry_thin_walled', 'normal'] : ['base', 'base_color', 'diffuse_roughness', 'metalness', 'specular', 'specular_color', 'specular_roughness', 'specular_IOR', 'specular_ior', 'specular_anisotropy', 'transmission', 'transmission_color', 'transmission_depth', 'transmission_scatter', 'subsurface', 'subsurface_color', 'subsurface_radius', 'coat', 'coat_color', 'coat_roughness', 'coat_IOR', 'emission_color', 'emission', 'opacity', 'thin_walled', 'normal', 'thin_film_thickness', 'thin_film_IOR']);
          for (const k of Object.keys(n.inputs || {})) if (!supported.has(k)) fail('UNSUPPORTED', `${key}/${k}`, 'surface input not yet implemented');
          const unsupportedLobes = open ? ['thin_film_weight'] : [];
          ...
          code = `materialFromClosure(${coated},${fields[5]}*${fields[6]},clamp(${opacity},0.0,1.0),normalize(${normal}))`; break;
        }
