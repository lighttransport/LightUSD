    ...
    if (compound) {
      ...
      switch (n.category) {
        ...
        case 'hextiledimage': {
          ...
          const descriptor=Object.hasOwn(imageDescriptors,file)&&imageDescriptors[file];if(!descriptor)fail('RESOURCE',key,`missing decoded image ${file}`);if(descriptor.udim)fail('UNSUPPORTED',key,'hextiledimage UDIM atlases are not implemented');if(n.colorspace&&normalizeColorSpace(n.colorspace)!==normalizeColorSpace(descriptor.colorspace))fail('SEMANTICS',key,'hextiledimage colorspace differs from decoded resource');
        ...
        case 'hextilednormalmap': {
          ...
          const descriptor=Object.hasOwn(imageDescriptors,file)&&imageDescriptors[file];if(!descriptor)fail('RESOURCE',key,`missing decoded image ${file}`);if(descriptor.udim)fail('UNSUPPORTED',key,'hextilednormalmap UDIM atlases are not implemented');if(n.colorspace&&normalizeColorSpace(n.colorspace)!==normalizeColorSpace(descriptor.colorspace))fail('SEMANTICS',key,'hextilednormalmap colorspace differs from decoded resource');
          ...
          const uv=ins.texcoord?x('texcoord',undefined,'vector2'):'ctx.uv',tiling=x('tiling',[1,1],'vector2'),coord=`(${uv}*${tiling})`,ddx=`(ctx.uvDx*${tiling})`,ddy=`(ctx.uvDy*${tiling})`,normal=ins.normal?x('normal',undefined,'vector3'):'ctx.normal',tangent=ins.tangent?x('tangent',undefined,'vector3'):'ctx.tangent',bitangent=ins.bitangent?x('bitangent',undefined,'vector3'):'ctx.bitangent',fill=`vec4f(${fallback},0.0)`;
          code=`imageHextileNormal(${descriptor.offset}u,vec2u(${descriptor.width}u,${descriptor.height}u),${descriptor.levels}u,${coord},${ddx},${ddy},${x('rotation',1,'float')},${x('rotationrange',[0,360],'vector2')},${x('scale',1,'float')},${x('scalerange',[.5,2],'vector2')},${x('offset',1,'float')},${x('offsetrange',[0,1],'vector2')},${x('falloff',.5,'float')},${x('strength',1,'float')},${x('flip_g',false,'boolean')},${normal},${tangent},${bitangent},vec2u(${address('uaddressmode')},${address('vaddressmode')}),${filter==='linear'},${fill})`;break;
        }
