...
class MaterialXRenderer extends EventTarget {
  ...
  resize(force = false) {
    ...
    const pathBytes = ['path-physical', 'path-spectral'].includes(this.mode) ? width * height * 128 : 128;
  ...
  async capture({ format = 'exr' } = {}) {
    ...
    try {
      ...
      const metadata = { mode: this.mode, referenceReady: false, colorSpace: spectral ? 'CIE XYZ' : 'linear Rec.709', spectral: spectral ? { wavelengthRange: [360,830], sampling: 'uniform hero wavelength', observer: 'CIE 1931 2 degree, 1 nm table', rgbUplift: 'PBRT v3 bases; illuminant white normalized to CIE Y=1', overrides: this.scene.materials.map(m=>m.spectra || {}) } : null, limitations: [...(spectral ? [] : ['RGB transport']), ...(this.mode === 'path-preview' ? ['12-bounce preview limit'] : ['three-level medium stack', 'single-scattering GGX', 'authored normals only where connected; geometric fallback']), 'approximate Standard Surface mapping', 'bounded closure composition; approximate MaterialX subsurface and hair fiber lobes', 'scalar bump height fallback; no texture finite differences', 'path textures use level zero', 'linear triangle displacement refinement, not Catmull-Clark', 'RGB graph working space is not ACEScg'], width, height, samples, adapter: this.stats.adapter, provenance: this.scene.provenance };
      metadata.seed=this.options.seed;
      metadata.limitations=metadata.limitations.map(v=>v==='bounded closure composition; approximate MaterialX subsurface and hair fiber lobes'?'16 surface lobes; no full BSDF-over-BSDF layering, exact BSSRDF, or Marschner hair':v);
      metadata.limitations=metadata.limitations.map(v=>v==='scalar bump height fallback; no texture finite differences'?'central-difference bump; local planar position/UV offsets, at most two nested derivative nodes':v==='path textures use level zero'?'isotropic path texture footprint; no true ray differentials':v);
