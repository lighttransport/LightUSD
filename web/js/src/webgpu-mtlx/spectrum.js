...
`;
  ...
  code += `fn spectralMaterial(input:Lobe,id:u32,lambda:f32)->Lobe { var m=input;m.base=vec3f(rgbSpectrum(m.base,lambda,false));m.schlickColor82=vec3f(rgbSpectrum(m.schlickColor82,lambda,false));m.schlickColor90=vec3f(rgbSpectrum(m.schlickColor90,lambda,false));m.transmissionColor=vec3f(rgbSpectrum(m.transmissionColor,lambda,false));m.transmissionScatter=vec3f(rgbSpectrum(m.transmissionScatter,lambda,false));m.emission=vec3f(rgbSpectrum(m.emission,lambda,true));m.complexIOR=vec3f(rgbSpectrum(m.complexIOR,lambda,false));m.extinction=vec3f(rgbSpectrum(m.extinction,lambda,false));switch id { ${cases.join('\n')} default:{} }return m; }\n`;
