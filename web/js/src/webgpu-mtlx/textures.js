...
fn imageHextileUDIM(offset:u32,size:vec2u,levels:u32,coord:vec2f,ddx0:vec2f,ddy0:vec2f,rotation:f32,rotationRange:vec2f,scale:f32,scaleRange:vec2f,offsetAmount:f32,offsetRange:vec2f,falloff:f32,falloffContrast:f32,lumacoeffs:vec3f,grid:vec2u,linear:bool,fallback:vec4f)->vec4f {
  let t=mxHextileCoord(coord,ddx0,ddy0,rotation,rotationRange,scale,scaleRange,offsetAmount,offsetRange);let lod0=log2(max(1.0,max(length(t.ddx[0]*vec2f(size)),length(t.ddy[0]*vec2f(size)))));let lod1=log2(max(1.0,max(length(t.ddx[1]*vec2f(size)),length(t.ddy[1]*vec2f(size)))));let lod2=log2(max(1.0,max(length(t.ddx[2]*vec2f(size)),length(t.ddy[2]*vec2f(size)))));
  let c1=imageSampleUDIM(offset,size,levels,t.coords[0],grid,lod0,linear,fallback);let c2=imageSampleUDIM(offset,size,levels,t.coords[1],grid,lod1,linear,fallback);let c3=imageSampleUDIM(offset,size,levels,t.coords[2],grid,lod2,linear,fallback);let cw=vec3f(dot(c1.rgb,lumacoeffs),dot(c2.rgb,lumacoeffs),dot(c3.rgb,lumacoeffs));let w=mxHextileBlendWeights(mix(vec3f(1.0),cw,vec3f(falloffContrast)),t.weights,falloff);let aw=mxHextileBlendWeights(vec3f(1.0),t.weights,falloff);return vec4f(w.x*c1.rgb+w.y*c2.rgb+w.z*c3.rgb,aw.x*c1.a+aw.y*c2.a+aw.z*c3.a);
}
...
fn imageHextileNormalUDIM(offset:u32,size:vec2u,levels:u32,coord:vec2f,ddx0:vec2f,ddy0:vec2f,rotation:f32,rotationRange:vec2f,scale:f32,scaleRange:vec2f,offsetAmount:f32,offsetRange:vec2f,falloff:f32,strength:f32,flipG:bool,N:vec3f,T:vec3f,B:vec3f,grid:vec2u,linear:bool,fallback:vec4f)->vec3f {
  let t=mxHextileCoord(coord,ddx0,ddy0,rotation,rotationRange,scale,scaleRange,offsetAmount,offsetRange);let lod0=log2(max(1.0,max(length(t.ddx[0]*vec2f(size)),length(t.ddy[0]*vec2f(size)))));let lod1=log2(max(1.0,max(length(t.ddx[1]*vec2f(size)),length(t.ddy[1]*vec2f(size)))));let lod2=log2(max(1.0,max(length(t.ddx[2]*vec2f(size)),length(t.ddy[2]*vec2f(size)))));
  var n1=imageSampleUDIM(offset,size,levels,t.coords[0],grid,lod0,linear,fallback).rgb;var n2=imageSampleUDIM(offset,size,levels,t.coords[1],grid,lod1,linear,fallback).rgb;var n3=imageSampleUDIM(offset,size,levels,t.coords[2],grid,lod2,linear,fallback).rgb;if(flipG){n1.y=1.0-n1.y;n2.y=1.0-n2.y;n3.y=1.0-n3.y;}n1=2.0*n1-vec3f(1.0);n2=2.0*n2-vec3f(1.0);n3=2.0*n3-vec3f(1.0);
  let n=safeNormal(N,vec3f(0.0,0.0,1.0));let r1=-t.rotations.x;let r2=-t.rotations.y;let r3=-t.rotations.z;let t1=mxHextileAxisRotate(n,T,r1)*strength;let t2=mxHextileAxisRotate(n,T,r2)*strength;let t3=mxHextileAxisRotate(n,T,r3)*strength;let b1=mxHextileAxisRotate(n,B,r1)*strength;let b2=mxHextileAxisRotate(n,B,r2)*strength;let b3=mxHextileAxisRotate(n,B,r3)*strength;let n1w=safeNormal(t1*n1.x+b1*n1.y+n*n1.z,n);let n2w=safeNormal(t2*n2.x+b2*n2.y+n*n2.z,n);let n3w=safeNormal(t3*n3.x+b3*n3.y+n*n3.z,n);let w=mxHextileBlendWeights(vec3f(1.0),t.weights,falloff);let g=w.x*mxHextileNormalGradient(n,n1w)+w.y*mxHextileNormalGradient(n,n2w)+w.z*mxHextileNormalGradient(n,n3w);return safeNormal(n-g,n);
}
`;
