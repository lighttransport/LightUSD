...
export function refineDisplacementScene(scene, levels = 0, maxTriangles = 500_000) {
  ...
  const result={...scene,positions:[],normals:[],uvs:[],uvSets:undefined,tangents:[],colors:[],indices:[],materialIds:[]};
  function vertex(i, mat) {
    if(!Number.isInteger(i)||i<0||i*3+2>=scene.positions.length)throw new Error('Invalid displacement index');
    const p=Array.from(scene.positions.slice(i*3,i*3+3)),n=scene.normals?Array.from(scene.normals.slice(i*3,i*3+3)):null,uvSet=scene.uvSets?.[scene.materials[mat]?.uvIndex||0],uv=uvSet?Array.from(uvSet.slice(i*2,i*2+2)):scene.uvs?Array.from(scene.uvs.slice(i*2,i*2+2)):[0,0],t=scene.tangents?.length===scene.positions.length/3*4?Array.from(scene.tangents.slice(i*4,i*4+4)):[0,0,0,1],color=scene.colors?.length===scene.positions.length/3*4?Array.from(scene.colors.slice(i*4,i*4+4)):scene.colors?.length===scene.positions.length/3*3?[...scene.colors.slice(i*3,i*3+3),1]:[0,0,0,1];
    if(![...p,...(n||[]),...uv,...t,...color].every(v=>Number.isFinite(v)&&Number.isFinite(Math.fround(v))))throw new Error('Invalid displacement geometry');
    return {p,n,uv,t,color};
  }
  const midpoint=(a,b)=>({p:a.p.map((v,k)=>(v+b.p[k])*.5),n:normalize(a.n.map((v,k)=>(v+b.n[k])*.5)),uv:a.uv.map((v,k)=>(v+b.uv[k])*.5),t:[...normalize(a.t.slice(0,3).map((v,k)=>(v+b.t[k])*.5)),a.t[3]>=0?1:-1],color:a.color.map((v,k)=>(v+b.color[k])*.5)});
  function triangle(a,b,c,mat,depth) {
    ...
    for(const v of [a,b,c]){result.indices.push(result.positions.length/3);result.positions.push(...v.p);result.normals.push(...v.n);result.uvs.push(...v.uv);result.tangents.push(...v.t);result.colors.push(...v.color);}result.materialIds.push(mat);
  }
  for(let t=0;t<scene.indices.length/3;t++) {
    ...
    const [a,b,c]=Array.from(scene.indices.slice(t*3,t*3+3),i=>vertex(i,mat));const n=normalize(cross(sub(b.p,a.p),sub(c.p,a.p)));for(const v of [a,b,c])v.n=v.n?normalize(v.n):n;
...
export async function bakeDisplacement(scene, device) {
  ...
  const functions=scene.materials.map((doc,i)=>{
    ...
    const c=compileGraph(doc,{output:doc.displacementOutput,imageDescriptors,uvIndex:Number.isInteger(doc.uvIndex)&&doc.uvIndex>=0?doc.uvIndex:0});
  ...
  const count=refined.positions.length/3,data=new Float32Array(count*20);
  for(let i=0;i<count;i++)data.set([...refined.positions.slice(i*3,i*3+3),0,...refined.normals.slice(i*3,i*3+3),0,...refined.uvs.slice(i*2,i*2+2),refined.materialIds[Math.floor(i/3)],0,...(refined.colors.length?refined.colors.slice(i*4,i*4+4):[0,0,0,1]),...(refined.tangents.length?refined.tangents.slice(i*4,i*4+4):[0,0,0,1])],i*20);
  ...
  const module=device.createShaderModule({code:`${contextWGSL}\n${imageWGSL}\n${functions}
    struct BakeVertex {p:vec4f,n:vec4f,uv:vec4f,color:vec4f,tangent:vec4f}
    ...
    @compute @workgroup_size(64) fn bake(@builtin(global_invocation_id) id:vec3u){
      ...
      let ctx=ShadingContext(v.p.xyz,n,select(frame[0],t,hasT),bt,v.uv.xy,0,0,vec2f(0),vec2f(0),derivatives[0],derivatives[1],vec3f(0,0,1),v.color);var d=vec3f(0);
